...
// 绘制气泡
function drawBubble (ctx: CanvasRenderingContext2D, x: number, y: number, r: number, fillColor: RGBColorArray = [255, 255, 255], shadowColor: RGBColorArray = [255, 245, 196]) {
  ctx.beginPath()
  ctx.save()
  ctx.translate(x, y)
...
// 绘制指示箭头
function drawArrow (ctx: CanvasRenderingContext2D, x: number, y: number, r: number, direction: RockerDirection, fillColor: RGBColorArray = [255, 255, 255]) {
  ctx.save()
  ctx.beginPath()
...
// 边缘高光
function drawEdgeLight (ctx: CanvasRenderingContext2D, context: RockerContext, angle: number) {
  ctx.save()
  ...
  const len = context.baseRadius * 0.8 % 2 === 0 ? context.baseRadius * 0.8 : context.baseRadius * 0.8 + 1
  ...
  for (let i = 0; i < len / 2; i++) {
    ...
    if ((len / 2 - i) <= 5) {
      ...
      ctx.beginPath()
      ctx.arc(p1.x, p1.y, i * 0.1, 0, 2 * Math.PI)
      ctx.fill()
      ctx.beginPath()
      ctx.arc(p2.x, p2.y, i * 0.1, 0, 2 * Math.PI)
      ctx.fill()
...
// 绘制摇杆主体
function drawRocker (ctx: CanvasRenderingContext2D, context: RockerContext) {
...
export const createVirtualRocker = (options?: RockerOptions) => {
  ...
  // 计算摇杆手柄的位置
  function updateRockerPosition (x: number, y: number) {
  ...
  const handleChangeDirection = (direction: RockerDirection) => {
